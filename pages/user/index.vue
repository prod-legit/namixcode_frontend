...
const { data: employments } = await useAsyncData(
    "employments",
    async () => {
        const data = await $api<{ job_id: string }[]>("/user/employments");
        return data;
    },
    {
        server: false,
...
<template>
    <div class="__work-search flex">
            ...
            <WorkJob
                ...
                :hide-btn="
                    (pickedItemIndex !== null &&
                        pickedItemIndex !== undefined) ||
                    (employments &&
                        employments.find((iter) => iter.job_id === item?.id))
                "
                @click="
                    () =>
        ...
        <div
            class="overflow-auto p-0 bg-izumrud-700 w-0 transition-all duration-300 ease-in-out md:rounded-bl-3xl md:rounded-tl-3xl fixed md:relative inset-0 md:inset-auto"
                    ...
                    <div class="space-y-4 text-white">
                        ...
                        <UButton
                            v-if="
                                ...
                                pickedItem &&
                                !employments.find(
                                    (item) => item.job_id === pickedItem.id
                                )
                            "
                            size="lg"
                        ...
                        />
                        <p v-else>
                            Вы приняты на должность. Свяжитесь с компанией:
                            <NuxtLink :to="`mailto:${pickedItem.org.email}`"
                                >{{ pickedItem.org.email }}
                            </NuxtLink>
                        </p>
                    </div>
                </div>
